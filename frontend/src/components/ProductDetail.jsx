...
const ProductDetail = () => {
    ...
    const handleAddToCart = async () => {
        ...
        if (!token) { if(window.confirm("Sepete eklemek için giriş yapmalısınız. Giriş sayfasına gidilsin mi?")) { navigate('/login'); } return; }
        setAdding(true);
        try {
            ...
            else { alert("Ürün sepete eklenirken teknik bir sorun oluştu."); }
        } finally {
            setAdding(false);
        }
    };
    ...
    const handleFavoriteToggle = async () => {
        if (!isUserLoggedIn) { if(window.confirm("Favorilere eklemek için giriş yapmalısınız. Giriş sayfasına gidilsin mi?")) { navigate('/login'); } return; }
        setFavoriting(true);
        setError(null);
        try {
            ...
            console.error("Favori işlemi hatası:", err.response?.data || err);
            const errMsg = err.response?.data?.detail || "Favori işlemi sırasında bir hata oluştu.";
    ...
    const handleReviewSubmit = async (e) => {
        ...
        try {
            ...
            console.error("Yorum gönderme hatası:", err.response?.data || err);
            setReviewMessage(`Hata: ${err.response?.data?.detail || "Yorum gönderilemedi (Belki de zaten yorum yaptınız?)."}`);
    ...
    const galleryImages = product ? (product.images || product.watch_images || []) : [];
            ...
            <div className="row g-5">
                ...
                <div className="col-lg-6">{/* ... (Galeri kodları aynı) ... */}
                    <div className="ratio ratio-1x1 rounded-4 shadow-sm d-flex align-items-center justify-content-center bg-white border mb-3 overflow-hidden position-relative">
                        ...
                        {product.Stock < 5 && product.Stock > 0 && ( <span className="position-absolute top-0 end-0 m-3 badge bg-danger fs-6 shadow-sm">Son {product.Stock} Ürün</span> )}
                    </div>
                    <div className="d-flex gap-2 overflow-auto py-2 px-1" style={{scrollbarWidth: 'thin'}}>
                        {product.ImageUrl && ( <div onClick={() => setActiveImage(product.ImageUrl)} className={`rounded-3 bg-white border d-flex align-items-center justify-content-center flex-shrink-0 cursor-pointer ${activeImage === product.ImageUrl ? 'border-warning border-2' : ''}`} style={{width: '80px', height: '80px', cursor: 'pointer', transition: 'all 0.2s'}}> <img src={product.ImageUrl} alt="Kapak" style={{maxWidth:'90%', maxHeight:'90%', objectFit:'contain'}} /> </div> )}
                        {galleryImages.map((img, index) => ( <div key={img.ImageID || index} onClick={() => setActiveImage(img.ImageUrl)} className={`rounded-3 bg-white border d-flex align-items-center justify-content-center flex-shrink-0 cursor-pointer ${activeImage === img.ImageUrl ? 'border-warning border-2' : ''}`} style={{width: '80px', height: '80px', cursor: 'pointer', transition: 'all 0.2s'}}> <img src={img.ImageUrl} alt={`Galeri ${index}`} style={{maxWidth:'90%', maxHeight:'90%', objectFit:'contain'}} /> </div> ))}
                    </div>
                </div>

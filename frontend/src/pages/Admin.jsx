...
const Admin = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (err.response && (err.response.status === 401 || err.response.status === 403)) {
                alert(getErrorMessage(err, "Yetkiniz yok veya oturumunuz sona erdi. Lütfen tekrar giriş yapın."));
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleStatusUpdate = async (orderId, newStatus) => {
        ...
        try {
            ...
            alert(`Durum güncelleme başarısız: ${getErrorMessage(err, "Bilinmeyen bir hata oluştu.")}`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleReturnStatusUpdate = async (returnId, newStatus) => {
        ...
        try {
            ...
            console.error("İade durumu güncelleme hatası:", err);
            alert(`Güncelleme başarısız: ${getErrorMessage(err, "Bilinmeyen bir hata oluştu.")}`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleAddBrandSubmit = async (e) => {
        ...
        try {
            ...
            const errMsg = getErrorMessage(err, "Marka eklenirken bir hata oluştu.");
    ...
    const handleFileUpload = async (e) => {
        ...
        } catch (err) { console.error("Kapak resmi hatası:", err); alert(getErrorMessage(err, "Kapak resmi yüklenirken hata oluştu.")); }
    };
    ...
    const handleGalleryUpload = async (e) => {
        ...
        } catch (err) { console.error("Galeri yükleme hatası:", err); alert(getErrorMessage(err, "Bazı resimler yüklenemedi.")); }
    };
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            alert(`Ürün eklenirken hata oluştu: ${getErrorMessage(err)}`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleRemoveProduct = async (id) => {
        ...
        try {
            ...
            alert(getErrorMessage(err, "Silme işlemi başarısız."));
        }
    };
                                            ...
                                            <tr key={order.OrderID}>
                                                ...
                                                <td>
                                                    <div className="d-flex flex-column">
                                                        <span className="fw-bold text-dark">{order.user?.FullName || 'Misafir'}</span>
                                                        <span className="small text-muted">ID: {order.UserID}</span>
                                                    </div>
                                                </td>
                    ...
                    <div className="modal-body">
                        <div className="row">
                            ...
                            <div className="col-md-6">
                                ...
                                <p><strong>Kullanıcı:</strong> {order.user?.FullName} (ID: {order.UserID})</p>

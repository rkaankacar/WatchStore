...
const Cart = () => {
  ...
  const fetchCart = async () => {
    try {
      ...
      setError(getErrorMessage(err, "Sepet bilgileri alınamadı. Lütfen giriş yaptığınızdan emin olun."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (cartId) => {
    ...
    try {
      ...
      alert(getErrorMessage(err, "Ürün silinirken bir hata oluştu."));
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const handleQuantityChange = async (item, newQuantity) => {
    ...
    try {
      ...
      alert(getErrorMessage(err, "Miktar güncellenirken bir hata oluştu (Örn: Geçersiz miktar, stok yetersizliği)."));
    } finally {

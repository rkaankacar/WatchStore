...
export default function ReturnDetails() {
  ...
  useEffect(() => {
    const fetchReturns = async () => {
      try {
        ...
        console.error("İade talepleri alınamadı:", getErrorMessage(err));
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderProductDetails = (returnItem) => {
    const order = returnItem.order;
    if (!order || !order.order_details || order.order_details.length === 0) {
      return <p className="mb-1"><strong>Ürün:</strong> Bilgi Bulunamadı</p>;
    }

    let detailsToShow = order.order_details;

    // Eğer bu iade talebi belirli bir ürüne aitse (OrderDetailID varsa), sadece onu filtrele
    if (returnItem.OrderDetailID) {
      detailsToShow = order.order_details.filter(d => d.OrderDetailID === returnItem.OrderDetailID);
    }

    // Siparişteki (veya filtrelenmiş) ürünleri listeliyoruz
    return detailsToShow.map((detail, index) => (
      <p key={index} className="mb-1">
        <strong>Ürün {detailsToShow.length > 1 ? index + 1 : ''}:</strong>
        {/*
